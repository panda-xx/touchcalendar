...
*/
	...
	onMousemove : function(evt){
		...
		if (this.isMoving) {			
			...
			// 一気にcalendarWidthを超える移動が発生した場合に対応済み。
		    if ( readNum >= 1){
	            for (var i = 0; i < Math.abs(readNum); i++){
...
function readNextDays(date){
    ...
    for (var i = 0; i < 7; i++){
        // そのテーブル要素に入れるものを作成する
        new Ajax.Updater(dateString[i], '/calendar/addPopupDayNext/' + dateString[i],
                          {asynchronous:true,
                           evalScripts:true, 
                           parameters:'authenticity_token=' + encodeURIComponent(authToken)
                          }
...
function readBeforeDays(date){
    ...
    for (var i = 0; i < 7; i++){
        // そのテーブル要素に入れるものを作成する
        new Ajax.Updater(dateString[i], '/calendar/addPopupDayBefore/' + dateString[i],
                          {asynchronous:true,
                           evalScripts:true, 
                           parameters:'authenticity_token=' + encodeURIComponent(authToken)
                          }
